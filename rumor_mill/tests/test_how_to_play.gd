...
static func test_c_overlay_semi_transparent() -> bool:
	...
	var ok: bool = h.C_OVERLAY.a > 0.40 and h.C_OVERLAY.a < 0.75 and h.C_OVERLAY.r < 0.10
	h.free()
	return ok
...
static func test_c_title_is_gold() -> bool:
	var h := _make_htp()
	var ok: bool = h.C_TITLE.r > 0.85 and h.C_TITLE.g > 0.70 and h.C_TITLE.b < 0.20
	h.free()
	return ok
...
static func test_c_tab_active_amber() -> bool:
	...
	var ok: bool = h.C_TAB_ACTIVE.r > 0.45 and h.C_TAB_ACTIVE.b < 0.25
	h.free()
	return ok
...
static func test_c_tab_inactive_very_dark() -> bool:
	var h := _make_htp()
	var ok: bool = h.C_TAB_INACTIVE.r < 0.25 and h.C_TAB_INACTIVE.g < 0.20
	h.free()
	return ok
...
static func test_tab_controls_is_zero() -> bool:
	var ok: bool = HowToPlayScript.Tab.CONTROLS == 0
	return ok


static func test_tab_mechanics_is_one() -> bool:
	var ok: bool = HowToPlayScript.Tab.MECHANICS == 1
	return ok


static func test_tab_systems_is_two() -> bool:
	var ok: bool = HowToPlayScript.Tab.SYSTEMS == 2
	return ok
...
static func test_initial_current_tab_is_controls() -> bool:
	var h := _make_htp()
	var ok: bool = h._current_tab == HowToPlayScript.Tab.CONTROLS
	h.free()
	return ok
...
static func test_initial_tab_buttons_empty() -> bool:
	var h := _make_htp()
	var ok: bool = h._tab_buttons.is_empty()
	h.free()
	return ok
...
static func test_initial_content_boxes_empty() -> bool:
	var h := _make_htp()
	var ok: bool = h._content_boxes.is_empty()
	h.free()
	return ok
