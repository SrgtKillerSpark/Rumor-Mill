...
static func test_c_bg_dark_semi() -> bool:
	var ccp := _make_ccp()
	var ok: bool = ccp.C_BG.r < 0.15 and ccp.C_BG.a < 0.90
	ccp.free()
	return ok
...
static func test_c_key_gold() -> bool:
	var ccp := _make_ccp()
	var ok: bool = ccp.C_KEY.r > 0.85 and ccp.C_KEY.g > 0.70 and ccp.C_KEY.b < 0.20
	ccp.free()
	return ok
...
static func test_c_active_green() -> bool:
	var ccp := _make_ccp()
	var ok: bool = ccp.C_ACTIVE.g > 0.90 and ccp.C_ACTIVE.r < 0.50
	ccp.free()
	return ok
...
static func test_mode_explore_is_zero() -> bool:
	var ok: bool = ContextControlsPanelScript.Mode.EXPLORE == 0
	return ok


static func test_mode_rumor_panel_is_one() -> bool:
	var ok: bool = ContextControlsPanelScript.Mode.RUMOR_PANEL == 1
	return ok


static func test_mode_journal_is_two() -> bool:
	var ok: bool = ContextControlsPanelScript.Mode.JOURNAL == 2
	return ok


static func test_mode_social_graph_is_three() -> bool:
	var ok: bool = ContextControlsPanelScript.Mode.SOCIAL_GRAPH == 3
	return ok


static func test_mode_paused_is_four() -> bool:
	var ok: bool = ContextControlsPanelScript.Mode.PAUSED == 4
	return ok
...
static func test_mode_bindings_count() -> bool:
	var ccp := _make_ccp()
	var ok: bool = ccp.MODE_BINDINGS.size() == 5
	ccp.free()
	return ok
...
static func test_initial_help_btn_null() -> bool:
	var ccp := _make_ccp()
	var ok: bool = ccp._help_btn == null
	ccp.free()
	return ok
...
static func test_initial_current_mode_explore() -> bool:
	var ccp := _make_ccp()
	var ok: bool = ccp._current_mode == ContextControlsPanelScript.Mode.EXPLORE
	ccp.free()
	return ok
...
static func test_initial_controls_ref_null() -> bool:
	var ccp := _make_ccp()
	var ok: bool = ccp._controls_ref == null
	ccp.free()
	return ok
...
static func test_initial_has_actions_true() -> bool:
	var ccp := _make_ccp()
	var ok: bool = ccp._has_actions == true
	ccp.free()
	return ok
...
static func test_initial_has_whispers_true() -> bool:
	var ccp := _make_ccp()
	var ok: bool = ccp._has_whispers == true
	ccp.free()
	return ok
...
static func test_initial_action_indicators_empty() -> bool:
	var ccp := _make_ccp()
	var ok: bool = ccp._action_indicators.is_empty()
	ccp.free()
	return ok
