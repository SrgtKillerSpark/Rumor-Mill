...
static func _make_world() -> Node2D:
...
static func test_tile_size() -> bool:
	if WorldScript.TILE_SIZE != Vector2i(64, 32):
		push_error("test_tile_size: expected Vector2i(64,32), got %s" % str(WorldScript.TILE_SIZE))
		return false
	return true


static func test_grid_dimensions() -> bool:
	if WorldScript.GRID_W != 48:
		push_error("test_grid_dimensions: GRID_W expected 48, got %d" % WorldScript.GRID_W)
		return false
	if WorldScript.GRID_H != 48:
		push_error("test_grid_dimensions: GRID_H expected 48, got %d" % WorldScript.GRID_H)
		return false
	return true


static func test_schedule_slots() -> bool:
	if WorldScript.SCHEDULE_SLOTS != 6:
		push_error("test_schedule_slots: expected 6, got %d" % WorldScript.SCHEDULE_SLOTS)
		return false
	return true
...
static func test_faction_schedules_has_merchant() -> bool:
	return WorldScript.FACTION_SCHEDULES.has("merchant")


static func test_faction_schedules_has_noble() -> bool:
	return WorldScript.FACTION_SCHEDULES.has("noble")


static func test_faction_schedules_has_clergy() -> bool:
	return WorldScript.FACTION_SCHEDULES.has("clergy")


static func test_merchant_schedule_is_non_empty() -> bool:
	var sched: Array = WorldScript.FACTION_SCHEDULES["merchant"]
	if sched.is_empty():
...
static func test_noble_schedule_is_non_empty() -> bool:
	var sched: Array = WorldScript.FACTION_SCHEDULES["noble"]
	if sched.is_empty():
...
static func test_clergy_schedule_is_non_empty() -> bool:
	var sched: Array = WorldScript.FACTION_SCHEDULES["clergy"]
	if sched.is_empty():
...
static func test_all_schedule_entries_are_strings() -> bool:
	for faction in WorldScript.FACTION_SCHEDULES:
		for entry in WorldScript.FACTION_SCHEDULES[faction]:
