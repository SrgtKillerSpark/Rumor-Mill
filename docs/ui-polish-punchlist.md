...
| 8 | ~~**S4 faction phase bars appear instantly (no tween).** When a phase fires, the bar snaps from 0 to full width. Should animate over ~0.5s.~~ **CONFIRMED ALREADY FIXED** — `_faction_bar_tweens` dict + 0.5 s tween already present in `_update_faction_bars()` (lines 448–453). No code change needed. | scenario4_hud.gd:373 | Small |
| 9 | ~~**Rival gap bar divides by magic constant 30.** If reputation gap exceeds 30, bar is clamped at 100%. No documentation on why 30.~~ **RESOLVED SPA-1681** — Design review confirmed divisor 30 (S4) and 30/20 (S5) are intentional balance values. Linear clamp kept (discrete 4-bucket output). Code comments added to both files. | objective_hud.gd:1010, objective_hud_metrics.gd:222 | Small |
...
| # | Issue | File : Line(s) | Severity |
|---|-------|---------------|----------|
| 22 | ~~**Journal slide animation uses hardcoded 40px offset.** Not viewport-relative; feels different at different scales.~~ **RESOLVED SPA-1683** — `40.0` replaced with `vp_h * 0.056` (≈40px at 720p). | journal.gd:176-177 | Small |
...
| 24 | ~~**Rumor panel subject portrait hardcoded to 48x60px.** No DPI scaling.~~ **RESOLVED SPA-1683** — `Vector2(48, 60)` replaced with `clamp(vp_h * 0.0667, 40, 64) × clamp(vp_h * 0.0833, 50, 72)` (≈48×60px at 720p). | rumor_panel_subject_list.gd:172 | Small |
...
| 44 | ~~**Loading tips label `custom_minimum_size` is 640x60 with `fit_content=true`.** Long tips can exceed 60px height.~~ **RESOLVED SPA-1683** — `fit_content = false`, `scroll_active = true`; label height stays at 60px min and scrolls for long tips. | loading_tips.gd:82-83 | Small |
